...
class App(QDialog):
    ...
    def createHorizontalLayout(self):
        ...
        # 新增识别结果保存
        feature_saved = QPushButton("FR Result Save", self)
        feature_saved.clicked.connect(self.feature_result_save)
        panel_layout.addWidget(feature_saved, 3, 0, 1, 1)
        ...
        panel_layout.addWidget(self.featureListWidget, 4, 0, 1, 1)
    ...
    def feature_result_save(self):
        feature_dict = {}
        if self.ais_shape and self.file_name and len(self.features_list) != 0:
            for item in self.features_list:
                temp_dict = {"faces":item.faces, "bottoms":item.bottoms}
                feature_dict.setdefault(item.name, []).append(temp_dict)
        
        # 获得文件名
        _file_name = os.path.basename(self.file_name)

        # 默认保存路径
        default_dir = os.path.join(current_dir, "FR_result")
        os.makedirs(default_dir, exist_ok=True)
        default_file_path = os.path.join(default_dir, f"{_file_name}.json")
        ...
        if file_path:
            try:
                data_to_save = {_file_name: feature_dict}
                with open(file_path, 'w', encoding='utf-8') as f:
                    json.dump(data_to_save, f, ensure_ascii=False, indent=4)
                QMessageBox.information(self, "Success! ", f"Feature data has been saved to: {file_path}")
            except Exception as e:
